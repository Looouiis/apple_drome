...
const EditableRemoteName: FC<{
	remoteName: string;
	onRemoteNameChange: (newName: string) => void;
}> = ({ remoteName: remoteName, onRemoteNameChange: onPlaylistNameChange }) => {
	const [editing, setEditing] = useState(false);
	const [newName, setNewName] = useState(remoteName);
		...
		<Heading className={styles.title}>
			...
			{editing && (
				<TextField.Root
					...
					onKeyDown={(e) => {
						if (e.key === "Enter") {
							if (newName !== remoteName) onPlaylistNameChange(newName);
							setEditing(false);
						}
					}}
					onBlur={() => {
...
export const Component: FC = () => {
	...
	const remote = useLiveQuery(() => db.remote.get(Number(param.id)));
	const { t } = useTranslation();
	const remoteViewRef = useRef<HTMLDivElement>(null);
	const remoteViewScroll = useScroll({
		container: remoteViewRef,
	});
	const remoteCoverSize = useMotionTemplate`clamp(6em,calc(12em - ${remoteViewScroll.scrollY}px),12em)`;
	const remoteInfoGapSize = useMotionTemplate`clamp(var(--space-1), calc(var(--space-4) - ${remoteViewScroll.scrollY}px / 5), var(--space-4))`;
	...
	// 		.map((v) => v.id)
	// 		.filter((v) => !remote?.songIds.includes(v))
	// 		.reverse();
	...
	const onPlayList = useCallback(
		async (songIndex = 0, shuffle = false) => {
			...
			const collected = await db.songs
				.toCollection()
				.filter((v) => remote.songIds.includes(v.id))
				.toArray();
			if (shuffle) {
				...
				collected.sort((a, b) => {
					return (
						remote.songIds.indexOf(a.id) - remote.songIds.indexOf(b.id)
					);
				});
			}
		...
		<PageContainer>
			<Flex direction="column" height="100%">
				<Flex gap="4" direction="column" flexGrow="0" pb="4" mt="5">
					...
					<Flex align="end" gap="3">
						...
						<Flex
							...
							gap={remoteInfoGapSize.get()}
							asChild
						>
							<motion.div
								style={{
									gap: remoteInfoGapSize,
								}}
							>
								<EditableRemoteName
									remoteName={remote?.name || ""}
									onRemoteNameChange={(newName) =>
										db.playlists.update(Number(param.id), (obj) => {
											obj.name = newName;
										})
									}
				...
				>
					{remote?.songIds && (
						<ViewportList
							items={remote.songIds}
							viewportRef={remoteViewRef}
						>
							{(songId, index) => (
								<PlaylistSongCard
